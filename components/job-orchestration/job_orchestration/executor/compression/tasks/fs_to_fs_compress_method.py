...
from job_orchestration.executor.compression.celery_compression import app  # type: ignore
...
def compress(
    ...
    archives_dir = Path(job_output_config["archives_dir"]).resolve()
    ...
    if 0 != return_code:
        logger.error(f"Failed to compress, return_code={return_code}")
    else:
