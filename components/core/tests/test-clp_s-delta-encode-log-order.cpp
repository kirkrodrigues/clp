...
public:
    SimpleFilterClass() = default;

    // Default copy & move constructors and assignment operators
    SimpleFilterClass(SimpleFilterClass const&) = default;
    SimpleFilterClass(SimpleFilterClass&&) noexcept = default;
    auto operator=(SimpleFilterClass const&) -> SimpleFilterClass& = default;
    auto operator=(SimpleFilterClass&&) noexcept -> SimpleFilterClass& = default;

    virtual ~SimpleFilterClass() = default;

    void init(
            [[maybe_unused]] clp_s::SchemaReader* reader,
            std::vector<clp_s::BaseColumnReader*> const& column_readers
    ) override {
        m_column_readers = column_readers;
    }

    auto filter([[maybe_unused]] uint64_t cur_message) -> bool override { return true; }
...
TEST_CASE("clp-s-delta-encode-log-order", "[clp-s][delta-encode-log-order]") {
    ...
    auto log_event_idx_node_id = mpt->get_metadata_field_id(
            static_cast<char const*>(clp_s::constants::cLogEventIdxName)
    );
