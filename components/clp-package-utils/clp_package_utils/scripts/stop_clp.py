"""Script for stopping the CLP package."""
...
def main(argv: List[str]) -> int:
    ...
    component_name = parsed_args.component_name if parsed_args.component_name else ""
    ...
    try:
        ...
        if not (
            logs_dir.exists() and logs_dir.is_dir() and instance_id_file_path.exists()
        ):
            # No instance ID file, so nothing to do
            return 0
        with instance_id_file_path.open("r") as f:
            instance_id = f.readline()

        if component_name in {"", WORKER_COMPONENT_NAME}:
            _stop_container(f"clp-{WORKER_COMPONENT_NAME}-{instance_id}")
        if component_name in {"", SCHEDULER_COMPONENT_NAME}:
        ...
        if component_name in {"", RESULTS_CACHE_COMPONENT_NAME}:
            container_name = f"clp-{RESULTS_CACHE_COMPONENT_NAME}-{instance_id}"
            _stop_container(container_name)
        if component_name in {"", QUEUE_COMPONENT_NAME}:
        ...
        if component_name in {"", DB_COMPONENT_NAME}:
            _stop_container(f"clp-{DB_COMPONENT_NAME}-{instance_id}")
