...
def wait_for_database_to_init(container_name: str, clp_config: CLPConfig, timeout: int):
    ...
    mysqladmin_cmd = [
        ...
        "127.0.0.1",
        "-u",
        str(clp_config.database.username),
...
def start_db(instance_id: str, clp_config: CLPConfig, conf_dir: pathlib.Path):
    ...
    mounts = [
        ...
        DockerMount(
            DockerMountType.BIND,
            db_data_dir,
            pathlib.Path("/") / "var" / "lib" / "mysql",
        ),
        DockerMount(
            DockerMountType.BIND,
            db_logs_dir,
            pathlib.Path("/") / "var" / "log" / "mysql",
        ),
    ]
    cmd = [
        "docker",
    ...
    append_docker_port_settings_for_host_ips(
        clp_config.database.host,
        clp_config.database.port,
        3306,
        cmd,
    )
    ...
    if not wait_for_database_to_init(container_name, clp_config, 30):
        raise OSError(f"{DB_COMPONENT_NAME} did not initialize in time")
...
def create_db_tables(
    ...
    create_tables_cmd = [
        ...
        "--config",
        str(container_clp_config.logs_directory / db_config_filename),
    ]
...
def start_queue(instance_id: str, clp_config: CLPConfig):
    ...
    append_docker_port_settings_for_host_ips(
        clp_config.queue.host,
        clp_config.queue.port,
        5672,
        cmd,
    )
    for mount in mounts:
...
def start_scheduler(
    ...
    scheduler_cmd = [
        ...
        "--config",
        str(container_clp_config.logs_directory / container_config_filename),
    ]
...
def start_worker(
    ...
    container_start_cmd = [
        ...
        f"{container_clp_config.queue.host}:{container_clp_config.queue.port}",
        "-e",
        f"RESULT_BACKEND=rpc://"
        f"{container_clp_config.queue.username}:{container_clp_config.queue.password}"
        f"@{container_clp_config.queue.host}:{container_clp_config.queue.port}",
        "-e",
        f"CLP_HOME={CONTAINER_CLP_HOME}",
        "-e",
        f"CLP_DATA_DIR={container_clp_config.data_directory}",
        "-e",
        f"CLP_ARCHIVE_OUTPUT_DIR={container_clp_config.archive_output.directory}",
        "-e",
        f"CLP_LOGS_DIR={container_clp_config.logs_directory}",
        "-u",
        f"{os.getuid()}:{os.getgid()}",
        "--mount",
        str(mounts.clp_home),
    ...
    worker_cmd = [
        ...
        "WARNING",
        "-Q",
        f"{QueueName.COMPRESSION},{QueueName.SEARCH}",
    ]
...
def main(argv):
    ...
    worker_args_parser.add_argument(
        "--num-cpus",
        type=int,
        default=0,
