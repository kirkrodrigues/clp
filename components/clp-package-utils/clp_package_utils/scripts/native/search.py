...
def create_and_monitor_job_in_db(
    ...
    with closing(sql_adapter.create_connection(True)) as db_conn, closing(
        ...
        # Create job
        db_cursor.execute(
            "INSERT INTO `search_jobs` (`search_config`) VALUES (%s)",
        ...
        while True:
            # Get next `limit` rows
            query_conditions = query_base_conditions + [
                f"`pagination_id` >= {next_pagination_id}",
            ]
            query = f"""
        ...
        while not job_complete:
            db_cursor.execute(
                f"SELECT `status`, `status_msg` FROM `search_jobs` WHERE `id` = {job_id}",
            )
