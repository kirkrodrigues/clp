...
class DockerMount:
    ...
    def __str__(self) -> str:
        mount_str = (
            f"type={DOCKER_MOUNT_TYPE_STRINGS[self.__type]},src={self.__src},"
            f"dst={self.__dst}"
        )
...
def get_clp_home():
    ...
    if clp_home is None:
        raise ValueError(
            "CLP_HOME is not set and could not be determined automatically.",
        )
...
def check_dependencies():
    ...
    try:
        ...
        raise OSError("docker cannot run without superuser privileges (sudo).")
...
def is_path_already_mounted(
    ...
    try:
        container_path_relative_to_mounted_root = container_path.relative_to(
            mounted_container_root,
        )
    except ValueError:
        return False
...
def validate_credentials_file_path(
    ...
    if not credentials_file_path.exists():
        if (
            ...
            raise ValueError(
                f"Credentials file path '{credentials_file_path}' does not exist.",
            )
    elif not credentials_file_path.is_file():
        raise ValueError(
            f"Credentials file path '{credentials_file_path}' is not a file.",
        )
...
def validate_results_cache_config(
    ...
    try:
        ...
        raise ValueError(
            f"{RESULTS_CACHE_COMPONENT_NAME} data directory is invalid: {ex}",
        )
    ...
    try:
        ...
        raise ValueError(
            f"{RESULTS_CACHE_COMPONENT_NAME} logs directory is invalid: {ex}",
        )
